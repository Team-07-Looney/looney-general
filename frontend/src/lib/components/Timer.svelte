<script>
  ...
  const offset = tweened(0, { duration, easing });
  const rotation = tweened(0, { duration, easing });
...
<div class="flex flex-col justify-center items-center">
  ...
  <div
    class="flex flex-row justify-center items-center mb-4 mt-2 gap-2 text-lg"
  >
    {#if !isStarted}
      <button
        on:click={handleStart}
        class="bg-[#63648D] rounded-lg p-2 text-white min-w-[85px]"
        >Start</button
      >
    {:else}
      {#if isOver}
        <button
          ...
          class="rounded-lg p-2 min-w-[85px] mr-2 bg-indigo-200 border-black"
          >Reset</button
        >
      {:else if !isPaused}
        <button
          on:click={handlePause}
          class="bg-gray-300 rounded-lg p-2 text-black min-w-[85px] mr-2"
          >Pause</button
        >
      {:else}
        <button
      ...
      <form
        method="POST"
        action={`/categories/${categoryId}/habits/${habitId}?/createRecord`}
      >
        <button
          type="submit"
          class="bg-[#63648D] rounded-lg p-2 text-white min-w-[85px] ml-2"
          >Done</button
        >
      </form>
    {/if}
