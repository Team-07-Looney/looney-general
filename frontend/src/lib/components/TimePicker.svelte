...
<script>
  ...
  export let date =
    data && display24
      ? new Date(`01/01/1970 ${data}`)
      : data && !display24 && dataElements[0] < 60
        ? new Date(`01/01/1970 ${currentDate.getHours()}:${data}`)
        : data && !display24 && dataElements[0] >= 60
          ? new Date(
            `01/01/1970 ${
              currentDate.getHours() + Math.floor(dataElements[0] / 60)
            }:${dataElements[0] - Math.floor(dataElements[0] / 60) * 60}:${
              dataElements[1]
            }`,
          )
          : new Date();
  const timeStyle = display24 ? "short" : "medium";
  let currentMinutes =
    data && dataElements[0] >= 60
      ? (date.getHours() - currentDate.getHours()) * 60 + date.getMinutes()
      : data
        ? date.getMinutes()
        : 0;
  ...
  const dateChanged = (event) => {
    const { type, changedData } = event.detail;
    ...
    case "minute":
      if (changedData >= 60) {
        ...
        newDate = new Date(
          ...
          date.getMonth(),
          date.getDate(),
          display24 ? date.getHours() : currentDate.getHours(),
          changedData,
          date.getSeconds(),
        );
      }
...
<div class="text-{color}">
  ...
  <div class="flex items-center border-b border-{color} pb-2">
    ...
    <input
      ...
      class={classes}
      readonly
      value={dateChange && display24
        ? _date
        : dateChange && !display24
  ...
  <div class="touch-date-popup" on:click={clickedOutside} bind:this={popup}>
    <div>
      <div class="touch-date-wrapper">
        <div
          class="flex flex-row items-center gap-6 {display24
            ? "pl-3"
            : ""} mx-10"
        >
        ...
        <div class="touch-date-picker">
          {#if display24}
            <Switcher
              type="hour"
              data={HOURS}
              selected={date.getHours() + 1}
              on:dateChange={dateChanged}
            />
            <Switcher
              type="minute"
              data={MINUTES}
              selected={date.getMinutes() + 1}
              on:dateChange={dateChanged}
            />
          {:else}
            <Switcher
              type="minute"
              data={MINUTES}
              selected={data || minutesChange
                ? (date.getHours() - currentDate.getHours()) * 60 +
                  date.getMinutes() +
                  1
                : 1}
              on:dateChange={dateChanged}
            ...
            <Switcher
              type="second"
              data={SECONDS}
              selected={data || secondsChange ? date.getSeconds() + 1 : 1}
              on:dateChange={dateChanged}
            />
          {/if}
        </div>
