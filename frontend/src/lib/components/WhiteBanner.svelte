...
<div class="bg-white h-36 w-full pt-1">
  <div class="flex flex-row justify-between items-center relative z-10 pr-3">
    <a href={route} type="button" class="p-1.5 opacity-{displayBackButton}"
      ><img
        src="{imgExtraPath}../src/img/backButton.png"
        alt="back"
        class=" h-7"
      /></a
    >
    <button on:click={toggleSideMenu} class="p-1.5 opacity-{displayMenuButton}"
      ><img
        src="{imgExtraPath}../src/img/menuButton.png"
        alt="menu"
        class="h-6"
      /></button
    >
