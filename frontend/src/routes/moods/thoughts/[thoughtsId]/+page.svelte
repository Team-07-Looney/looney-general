<script>
  ...
  if (data.moodType === "Negative") {
    icon = "sad";
  } else if (data.moodType === "Positive") {
    icon = "happy";
  } else {
    icon = "neutral";
  }
...
<WhiteBanner
    ...
    route="/moods/thoughts"
    displayBackButton="1"
    imgExtraPath="../"
/>

<div class="bg-white mt-6 h-3/5 px-5 mx-6 pb-5 pt-2 flex flex-col rounded-xl gap-2">
    ...
    <div class="bg-[#EDEDED] w-full h-full rounded-xl px-3 py-3 overflow-y-scroll">
        <p class="text-xl">{data.thoughts[0].body}</p>
    </div>
</div>
