...
{#if data.filteredThoughtsByUser.length == 0}
  <div
    class="rounded-xl p-1 shadow-lg text-center pt-2 pb-2 ml-10 mr-10 mt-4 bg-accent"
  >
...
{:else}
  <div class="flex justify-center items-center pt-4">
    <div
      ...
      ml-2 h-96 items-center w-80 px-1 gap-2 mt-5"
    >
      {#each data.filteredThoughtsByUser as thought, i}
        <ThoughtItem
          thoughtId={thought.id}
          title={thought.title}
          date={data.thoughtsDate}
          iconType={data.moodType[i]}
        />
      {/each}
    </div>
  </div>
